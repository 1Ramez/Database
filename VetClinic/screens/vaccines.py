...
from queries.vaccine_queries import get_all_vaccines, insert_vaccine, update_vaccine, delete_vaccine
...
class VaccineCard(tk.Frame):
    CARD_W = 260
    CARD_H = 120

    def __init__(self, parent, vaccine_data, color_index, on_edit=None, on_delete=None, **kwargs):
        super().__init__(parent, bg=t.BG_CARD, highlightbackground=t.BORDER,
                         highlightthickness=1, relief="flat",
                         width=self.CARD_W, height=self.CARD_H, **kwargs)
        self.pack_propagate(False)

        vaccine_id, vaccine_name = vaccine_data
        ...
        tk.Button(btn_frame, text="✏", bg=t.BG_CARD, fg=t.TEXT_MUTED,
                  ...
                  command=lambda: on_edit(vaccine_id) if on_edit else None).pack(side="left", padx=(0, 4))

        tk.Button(btn_frame, text="🗑", bg=t.BG_CARD, fg=t.DANGER,
                  font=("Segoe UI", 13), relief="flat", borderwidth=0,
                  cursor="hand2", activebackground=t.BG_HOVER, activeforeground=t.DANGER,
                  command=lambda: on_delete(vaccine_id) if on_delete else None).pack(side="left")

        tk.Label(self, text=vaccine_name, bg=t.BG_CARD, fg=t.TEXT_PRIMARY,
...
class VaccineFormDialog(tk.Toplevel):
    def __init__(self, parent, title="Add Vaccine", vaccine_data=None, on_save=None):
        ...
        self._field(form, "Vaccine Name *", "vaccine_name")

        if vaccine_data:
            vaccine_id, vaccine_name = vaccine_data
            self.vars["vaccine_name"].set(vaccine_name or "")
    ...
    def _submit(self):
        ...
        if not data.get("vaccine_name"):
            messagebox.showwarning("Missing", "Vaccine Name is required.", parent=self)
            return
...
class VaccinesScreen(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(left, text="Vaccines", **t.PAGE_TITLE_STYLE).pack(anchor="w")
        self.subtitle_lbl = tk.Label(left, text="Loading...", **t.PAGE_SUBTITLE_STYLE)
        self.subtitle_lbl.pack(anchor="w")

        tk.Button(header, text="  + Add Vaccine  ", **t.BTN_PRIMARY,
    ...
    def _load_vaccines(self):
        self.subtitle_lbl.config(text="Loading...")
        threading.Thread(target=self._fetch, daemon=True).start()

    def _fetch(self):
        try:
            data = list(get_all_vaccines())
    ...
    def _on_loaded(self, data, error):
        ...
        self._all_vaccines = data
        self._render(self._all_vaccines)

    def _filter(self):
        q = self.search_var.get().lower()
        filtered = [v for v in self._all_vaccines if q in str(v[1]).lower()]
        self._render(filtered)

    def _render(self, vaccines):
        for w in self.cards_frame.winfo_children():
            w.destroy()

        n = len(vaccines)
        self.subtitle_lbl.config(text=f"{n} vaccine{'s' if n != 1 else ''} registered")

        for idx, vaccine in enumerate(vaccines):
            row, col = divmod(idx, self.COLUMNS)
            VaccineCard(self.cards_frame, vaccine_data=vaccine, color_index=idx,
    ...
    def _open_add(self):
        VaccineFormDialog(self, title="Add Vaccine", on_save=self._do_insert)

    def _open_edit(self, vaccine_id):
        vaccine = next((v for v in self._all_vaccines if str(v[0]) == str(vaccine_id)), None)
        if not vaccine:
            return
        VaccineFormDialog(self, title="Edit Vaccine", vaccine_data=vaccine,
                          on_save=lambda d: self._do_update(vaccine_id, d))

    def _confirm_delete(self, vaccine_id):
        vaccine = next((v for v in self._all_vaccines if str(v[0]) == str(vaccine_id)), None)
        name = vaccine[1] if vaccine else vaccine_id
        if messagebox.askyesno("Delete Vaccine", f"Delete '{name}'? This cannot be undone."):
            try:
    ...
    def _do_insert(self, data):
        try:
            insert_vaccine(data["vaccine_name"])
            self._load_vaccines()
        except Exception as exc:
            messagebox.showerror("Error", str(exc))

    def _do_update(self, vaccine_id, data):
        try:
            update_vaccine(vaccine_id, data["vaccine_name"])
            self._load_vaccines()
        except Exception as exc:
            messagebox.showerror("Error", str(exc))